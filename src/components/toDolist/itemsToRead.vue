    ...
    <div>
        <div class="toDolistMain">
            ...
            <div class="listContent">
                <div class="allcheck" @click="toggleSelection(toDoList.tasks)">全选</div>
                <div class="allread" @click="toggleSelection(toDoList.tasks)">全部已阅</div>
                <el-tabs v-model="activeName" @tab-click="handleClick">
                    <el-tab-pane :label="todoList" name="daiyue">
                            ...
                            <el-table ref="multipleTable" :data="toDoList.tasks" tooltip-effect="dark" style="width: 100%" :show-header="false" @selection-change="handleSelectionChange">
                                ...
                                <el-table-column label="标题" width="300">
                                    <template slot-scope="scope">
                                        <div class="tabletext">{{ scope.row.taskName }}</div>
                                    </template>
                                </el-table-column>
                                <el-table-column label="日期" width="120">
                                    <template slot-scope="scope">
                                        <div class="tabletime">{{ scope.row.handleTime }}</div>
                                    </template>
                                </el-table-column>
                            </el-table>
                        </div>
                    ...
                    <el-tab-pane :label="nomalList" name="yiyue">
                    ...
                    <el-tab-pane :label="saveList" name="daiyuesave">
...
<script>
import { getMyToDoList, toBeRead, getUser } from '@/api/todolist/index.js'
export default {
    data() {
        return {
            todoList: '待阅(0)',
                                ...
                                toBeRead(
                                    {
                                        ...
                                        (res) => {
                                            if (res.code == 200) {
                                                ...
                                                document.getElementById("tab-daiyue").innerHTML = `待阅(<span style="color:red">${res.data.tasks.length}</span>)`
                                            }
                                        }
                                    )
                                toBeRead(
