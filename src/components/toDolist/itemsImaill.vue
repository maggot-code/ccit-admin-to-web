    ...
    <div>
        <div class="toDolistMain">
            ...
            <div class="listContent">
                ...
                <div class="allread" @click="toggleSelection(toDoList.data)">全部已阅</div>
                        ...
                        <div v-if='toDoList.data && toDoList.data.length!=0'>
                            ...
                            <el-table ref="multipleTable" :data="toDoList.data" tooltip-effect="dark" style="width: 100%" :show-header="false" @selection-change="handleSelectionChange">
                                ...
                                <el-table-column label="日期" width="120">
                                    <template slot-scope="scope">
                                        <div class="tabletime">{{ scope.row.createtime.slice(0,10) }}</div>
                                    </template>
                                </el-table-column>
                            </el-table>
                        </div>
...
<script>
import { getMyToDoList, toBeMaill, getUser } from '@/api/todolist/index.js'
export default {
    data() {
        return {
            todoList: '邮件(0)',
                                ...
                                toBeMaill(
                                    {
                                        ...
                                        (res) => {
                                            if (res.code == 200) {
                                                this.toDoList = res
                                                document.getElementById("tab-daiyue").innerHTML = `邮件(<span style="color:red">${res.data.tasks.length}</span>)`
                                            }
                                        }
                                    )
                            }
