        ...
        <div class="toDolistMain">
            ...
            <div class="listContent">
                <el-tabs v-model="activeName" @tab-click="handleClick">
                    <el-tab-pane :label="todoList" name="daiban">
                        <div v-if='toDoList.tasks && toDoList.tasks.length!=0'>
                            <div class="list" v-for="(item,index) in toDoList.tasks" :key="index" @click="formatUrl(toDoList,item,toDoList.state)">
                                <div class="listIcon">
                                    <div v-if="item.typeName==='加密'" class="listIcond33838">{{item.typeName}}</div>
                                    <div v-else :class="index%2==0?'listIcon597af2':'listIconba7adc'">{{item.typeName}}</div>
                                </div>
                                <div class="listcontent">
                                    {{item.taskName}}
                                </div>
                        ...
                        <div v-else>
                            ...
                            <div class="listtext">暂时没有待办事项...</div>
                        </div>
                    </el-tab-pane>
                    <el-tab-pane :label="nomalList" name="yiban">
                        ...
                        <div v-else>
                            <div class="nolist"></div>
                            <div class="listtext">暂时没有已办事项...</div>
                        </div>
                    </el-tab-pane>
                    <el-tab-pane :label="saveList" name="save1">
...
<script>
import { getMyToDoList, getMyToDoList2, getUser } from '@/api/todolist/index'
export default {
    data() {
        return {
            todoList: '待办(0)',
                                ...
                                getMyToDoList2(
                                    {
                                        ...
                                        (res) => {
                                            if (res.code == 200) {
                                                ...
                                                document.getElementById("tab-daiban").innerHTML = `待办(<span style="color:red">${res.data.tasks.length}</span>)`
                                            }
                                        }
                                    )
                                getMyToDoList2(
