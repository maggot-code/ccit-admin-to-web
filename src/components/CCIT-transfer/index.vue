  ...
  >
        ...
        <template v-if="userList.length">
          ...
          >
            <span class="custom-tree-node" slot-scope="{ node, data }">
              ...
              <span class="text">{{ data.realName }}</span>
            </span>
          </el-tree>
        </template>
        <template v-else>
    ...
    <div class="transfer-pane">
      <div class="transfer-pane__tools">
        <span>已选</span>
        <div>
          <el-button v-if="userList.length" @click="checkAll" type="text">全选</el-button>
      ...
      <div class="transfer-pane__body shadow right-pane">
        <template>
          <div
            ...
            class="selected-item"
          >
            <span>{{ item.realName || item.fullName }}</span>
...
export default {
  ...
  props: {
    ...
    props: {
      type: Object,
      default: () => {
        return { value: "id", label: "realName", children: "children" };
      },
    },
    type: {
      type: String,
  ...
  methods: {
    ...
    checkAll() {
      let list = this.userList.map((o) => o.id);
      this.$refs.treeBox.setCheckedKeys(list);
      this.selectedTextData = this.userList;
      this.selectedData = this.selectedTextData.map((o) => o.id);
      this.$emit("input", this.selectedData);
      this.$emit("getValue", this.selectedData, this.selectedTextData);
    },
