<template>
  <div class="ccit-table-box" :class="[config.__config__.type]">
    <div
      class="CCIT-common-title"
    ...
    >
      <el-table-column width="50" align="center" label="序号">
        ...
        <template slot-scope="scope">
          <div class="row-action">
            <span class="index" :class="{ 'btn-disabled': disabled }">
              {{ scope.$index + 1 }}</span
            >
            <i
      ...
      >
        ...
        <template slot-scope="scope">
          ...
          <template
            v-if="
              ['select', 'checkbox', 'radio'].includes(head.__config__.ccitKey)
            "
          >
            <el-select
          ...
          <!-- 单行输入 -->
          <template v-else-if="head.__config__.ccitKey === 'comInput'">
            <el-input
              v-model="tableFormData[scope.$index][cindex].value"
              v-bind="getConfById(head.__config__.formId)"
              :rowIndex="scope.$index"
          ...
          <template v-else-if="head.__config__.ccitKey === 'calculate'">
            <calculate
              :rowIndex="scope.$index"
              :value="tableFormData[scope.$index][cindex].value"
              v-model="tableFormData[scope.$index][cindex].value"
              v-bind="getConfById(head.__config__.formId)"
              :formData="formData"
              @change="onFormDataChange(scope.$index, cindex, 'calculate')"
            >
            </calculate>
          </template>
          <!-- 其他 -->
          <component
            ...
            :formData="formData"
            @change="
              onFormDataChange(scope.$index, cindex, head.__config__.tag)
            "
          >
          </component>
          <div
...
export default {
  ...
  methods: {
    buildOptions() {
      this.tableData.forEach((cur) => {
        ...
        if (dyOptionsList.indexOf(config.ccitKey) > -1) {
          let isTreeSelect =
            config.ccitKey === "treeSelect" || config.ccitKey === "cascader";
    ...
    onFormDataChange(rowIndex, colIndex, tag) {
      ...
      if (["CCIT-Amount", "el-input-number", "calculate"].includes(tag)) {
        // 金额变动 更新数据 触发计算公式更新
        const newVal = this.tableFormData.map((row) =>
          row.reduce((p, c) => ((p[c.__vModel__] = c.value), p), {})
        );
    ...
    getEmptyRow(val) {
      return this.tableData.map((t) => {
        let res = {
          ...
          value: (val && val[t.__vModel__]) || t.__config__.defaultValue || 0,
          options:
            (t.__slot__ && t.__slot__.options && t.__slot__.options) || [], // 下拉 单选 多选
          valid: true,
    ...
    getCmpValOfRow(row, key) {
      // 获取数字相关组件的输入值
      const isNumCmp = (tag) =>
        ["fc-amount", "el-input-number", "el-slider"].includes(tag);
    ...
    reset() {
      this.tableData.map((t) => {
        let index = this.tableFormData[0].findIndex(
          (c) => c.vModel === t.vModel
        );
        if (index === -1) return;
