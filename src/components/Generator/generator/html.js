...
const tags = {
  ...
  'CCIT-UploadFz': function(el) {
    const tag = this['CCIT-UploadImg'](el)
    const buttonText = `buttonText="${el.buttonText}"`
    return addPropToTag(tag, buttonText)
  },
  'CCIT-Quill': function(el) {
    const { tag, vModel, placeholder } = attrBuilder(el)
