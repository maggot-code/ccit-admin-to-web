  ...
  methods: {
      ...
      return new Promise((resolve, reject) => {
        ...
        const loop = list => {
          for (let i = 0; i < list.length; i++) {
            ...
            if (this.$store.getters.hasTable && config.layout === "colFormItem" && !noVModelList.includes(config.ccitKey) && !e.__vModel__) {
              reject({ msg: `请选择${config.label}的字段名`, target: 1 })
              break
            }
            if (config.ccitKey === 'billRule') {
              if (!config.rule) {
                reject({ msg: '单据组件“选择规则”属性为必填项', target: 1 })
                break
              }
            }
            if (config.ccitKey === 'relationForm') {
              if (!e.modelId) {
            ...
            if (config.ccitKey === 'relationFlow' && !e.flowId) {
    ...
    shouldClone(to, from, target, event, conf) {
      ...
      if (conf.__config__.rowType === 'table') {
        if (noTableAllowList.includes(targetConf.__config__.ccitKey)) {
          // if (this.showTip) {
      ...
      if (conf.__config__.ccitKey === 'card') {
        if (targetConf.__config__.layout === 'rowFormItem') return false
      }
      return true
    },
    ...
    createIdAndKey(item) {
      ...
      if (config.layout === 'colFormItem') {
        if (!this.$store.getters.hasTable) {
          // 分割线和按钮不加vModel
          if (noVModelList.indexOf(config.ccitKey) > -1) return
          item.__vModel__ = `${config.ccitKey}Field${this.idGlobal}`
        } else {
          if (noVModelList.indexOf(config.ccitKey) > -1) return
          item.__vModel__ = ""
        }
      } else if (config.layout === 'rowFormItem') {
        if (config.ccitKey === 'table') {
          item.__vModel__ = `${config.ccitKey}Field${this.idGlobal}`;
        }
