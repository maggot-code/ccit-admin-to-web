<template>
  <el-row>
    ...
    <el-form-item label="占位提示" v-if="activeData.__config__.ccitKey !=='editor'">
    ...
    <el-form-item label="默认值">
      <com-select v-model="activeData.__config__.defaultValue" placeholder="选择默认值"
        v-if="activeData.__config__.ccitKey==='comSelect'" :multiple="activeData.multiple" />
      <dep-select v-model="activeData.__config__.defaultValue" placeholder="选择默认值"
        v-if="activeData.__config__.ccitKey==='depSelect'" :multiple="activeData.multiple" />
      <pos-select v-model="activeData.__config__.defaultValue" placeholder="选择默认值"
        v-if="activeData.__config__.ccitKey==='posSelect'" :multiple="activeData.multiple" />
      <user-select v-model="activeData.__config__.defaultValue" placeholder="选择默认值"
        v-if="activeData.__config__.ccitKey==='userSelect'" :multiple="activeData.multiple" />
      <dic-select v-model="activeData.__config__.defaultValue" placeholder="选择默认值"
        v-if="activeData.__config__.ccitKey==='dicSelect'" />
      <BillRule v-model="activeData.__config__.defaultValue" placeholder="选择默认值"
        v-if="activeData.__config__.ccitKey==='billRule'" />
      <el-input :value="setDefaultValue(activeData.__config__.defaultValue)" placeholder="请输入默认值"
        @input="onDefaultValueInput" v-if="activeData.__config__.ccitKey==='editor'" />
    </el-form-item>
...
export default {
  ...
  computed: {
    needMultiple() {
      return ['posSelect', 'userSelect', 'comSelect', 'depSelect'].indexOf(this.activeData.__config__.ccitKey) > -1
    },
  },
  created() { },
  methods: {
