<template>
  <el-row>
    ...
    <el-form-item label="计算精度">
      <el-input-number v-model="activeData.precision" placeholder="计算精度" :min="0"
      :max="5" :precision="0" /> 
    </el-form-item>
    <el-dialog title="编辑计算公式" custom-class="calc-dialog" :visible.sync="expDialogVisible"
      width="600px" class="CCIT-dialog CCIT-dialog_center" @open="onOpen" append-to-body>
      <div class="calc-box">
        ...
        <div style="margin: 10px 0;">
          <span>计算符号：</span>
          <span v-for="item in ['+', '-', '×', '÷', 'sum', '(', ')']" class="calc-btn" :key="item"
    ...
    calculateCmps() {
      ...
      const loop = (data, parent) => {
        ...
        if (data.__config__ && data.__config__.ccitKey) {
          if (['numInput', 'calculate'].includes(data.__config__.ccitKey)) {
            const isTableChild = parent && parent.__config__ && parent.__config__.ccitKey === 'table'
            calcList.push({
