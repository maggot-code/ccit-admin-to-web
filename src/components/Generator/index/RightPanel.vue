        ...
        <el-form v-show="currentTab==='field' && showField" size="small" label-width="90px"
          ...
          <template v-if="activeData.__config__">
            <template v-if="$store.getters.hasTable">
              <template v-if="activeData.__config__.ccitKey==='table'">
            ...
            <CCITComInput v-if="activeData.__config__.ccitKey==='comInput'"
              :active-data="activeData" />
            <CCIT-textarea v-if="activeData.__config__.ccitKey==='textarea'"
              :active-data="activeData" />
            <CCITText v-if="activeData.__config__.ccitKey==='CCITText'" :active-data="activeData" />
            <CCITNumInput v-if="activeData.__config__.ccitKey==='numInput'"
              :active-data="activeData" />
            <CCITAmount v-if="activeData.__config__.ccitKey==='CCITAmount'"
              :active-data="activeData" />
            <CCITPsdInput v-if="activeData.__config__.ccitKey==='PsdInput'"
              :active-data="activeData" />
            <CCITRadio v-if="activeData.__config__.ccitKey==='radio'" :active-data="activeData" />
            <CCITCheckbox v-if="activeData.__config__.ccitKey==='checkbox'"
              :active-data="activeData" />
            <CCITSelect v-if="activeData.__config__.ccitKey==='select'" :active-data="activeData" />
            <CCITCascader v-if="activeData.__config__.ccitKey==='cascader'"
              :active-data="activeData" />
            <CCITTime v-if="activeData.__config__.ccitKey==='time'" :active-data="activeData" />
            <CCITTimeRange v-if="activeData.__config__.ccitKey==='timeRange'"
              :active-data="activeData" />
            <CCITDate v-if="activeData.__config__.ccitKey==='date'" :active-data="activeData" />
            <CCITDateRange v-if="activeData.__config__.ccitKey==='dateRange'"
              :active-data="activeData" />
            <CCITColorPicker v-if="activeData.__config__.ccitKey==='colorPicker'"
              :active-data="activeData" />
            <CCITRate v-if="activeData.__config__.ccitKey==='rate'" :active-data="activeData" />
            <CCITSwitch v-if="activeData.__config__.ccitKey==='switch'" :active-data="activeData" />
            <CCITSlider v-if="activeData.__config__.ccitKey==='slider'" :active-data="activeData" />
            <CCITDivider v-if="activeData.__config__.ccitKey==='divider'"
              :active-data="activeData" />
            <CCITUploadFz v-if="activeData.__config__.ccitKey==='uploadFz'"
              :active-data="activeData" />
            <CCITUploadImg v-if="activeData.__config__.ccitKey==='uploadImg'"
              :active-data="activeData" />
            <CCITComRight v-if="isCommon" :active-data="activeData" />
            <CCITAddress v-if="activeData.__config__.ccitKey==='address'"
              :active-data="activeData" />
            <TreeSelect v-if="activeData.__config__.ccitKey==='treeSelect'"
              :active-data="activeData" />
            <GroupTitle v-if="activeData.__config__.ccitKey==='groupTitle'"
              :active-data="activeData" />
            <RelationForm v-if="activeData.__config__.ccitKey==='relationForm'"
              :active-data="activeData" v-on="$listeners" />
            <RelationFormAttr v-if="activeData.__config__.ccitKey==='relationFormAttr'"
              :active-data="activeData" ref="relationFormAttr" />
            <RelationFlow v-if="activeData.__config__.ccitKey==='relationFlow'"
              :active-data="activeData" v-on="$listeners" />
            <RelationFlowAttr v-if="activeData.__config__.ccitKey==='relationFlowAttr'"
              :active-data="activeData" ref="relationFlowAttr" />
            <CCITCalculate v-if="activeData.__config__.ccitKey==='calculate'"
            ...
            <template v-if="isSystem">
              ...
              <el-form-item label="是否隐藏" v-if="activeData.__config__.ccitKey !=='billRule'">
                <el-switch v-model="activeData.__config__.noShow" />
              </el-form-item>
              <el-form-item label="选择规则" v-if="activeData.__config__.ccitKey==='billRule'">
            ...
            <template v-if="activeData.__config__.ccitKey==='card'">
            ...
            <!-- row 布局 -->
            <template v-if="activeData.__config__.ccitKey==='row'">
...
import CCITComInput from './RightComponents/ComInput'
import CCITTextarea from './RightComponents/Textarea'
import CCITText from './RightComponents/CCITText'
import CCITNumInput from './RightComponents/NumInput'
import CCITAmount from './RightComponents/CCITAmount'
import CCITPsdInput from './RightComponents/PsdInput'
import CCITRadio from './RightComponents/Radio'
import CCITCheckbox from './RightComponents/Checkbox'
import CCITSelect from './RightComponents/Select'
import CCITCascader from './RightComponents/Cascader/index'
import CCITTime from './RightComponents/Time'
import CCITTimeRange from './RightComponents/TimeRange'
import CCITDate from './RightComponents/Date'
import CCITDateRange from './RightComponents/DateRange'
import CCITColorPicker from './RightComponents/ColorPicker'
import CCITRate from './RightComponents/Rate'
import CCITSwitch from './RightComponents/Switch'
import CCITSlider from './RightComponents/Slider'
import CCITDivider from './RightComponents/Divider'
import CCITUploadFz from './RightComponents/UploadFz'
import CCITUploadImg from './RightComponents/UploadImg'
import CCITComRight from './RightComponents/ComRight'
import CCITAddress from './RightComponents/Address'
...
import CCITCalculate from './RightComponents/Calculate'
...
export default {
  ...
  computed: {
    ...
    isCommon() {
      return commonRightList.indexOf(this.activeData.__config__.ccitKey) > -1
    },
    isSystem() {
      return systemList.indexOf(this.activeData.__config__.ccitKey) > -1
    },
  },
  watch: {
    formConf: {
    ...
    fieldChange(val) {
      ...
      const loop = list => {
        for (let i = 0; i < list.length; i++) {
          ...
          if (config && config.ccitKey != 'table' && config.children && Array.isArray(config.children)) {
