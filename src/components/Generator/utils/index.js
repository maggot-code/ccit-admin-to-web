...
export const validExp = (expressions, mergeNum = true) => {
  ...
  const arr = temp.filter(t => !'()sum'.includes(t))
  // 去括号后 length应该为奇数  并且第一个字符和最后一个字符应该为数字而非计算符号
  if(temp.findIndex(t => t === 'sum') > -1) {
    if(temp.length % 2 === 1 || arr.length % 2 === 0 || Number.isNaN(+arr[0]) || Number.isNaN(+arr[arr.length - 1]))
    return false
  } else if (temp.length % 2 === 0 || arr.length % 2 === 0 || Number.isNaN(+arr[0]) || Number.isNaN(+arr[arr.length - 1])) {
    return false
  }
 ...
 */
export const calcRPN = (rpnExps, precision) => {
  ...
  const calc = (x, y, type) => {
    ...
    switch (type) {
      case '+':
        return (a1 + a2).toFixed(precision);
      case '-':
        return (a1 - a2).toFixed(precision);
      case '×':
        return (a1 * a2).toFixed(precision);
