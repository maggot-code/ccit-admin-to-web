      ...
      <div class="search-box">
        ...
        >
          <el-table-column align="center" label="拖动" width="50">
            <template>
              <i
                class="drag-handler icon-ym icon-ym-darg"
        ...
        <div class="setting-box" v-show="currentTab === 'column'">
          <el-form :model="columnData" label-width="80px">
            ...
            <div class="typeList">
              ...
              >
                ...
                >
                  ...
                  <div
                    class="icon-checked"
                    v-if="columnData.type == item.value"
                  >
            ...
            <el-form-item label="排序字段">
              <el-select
                v-model="columnData.defaultSidx"
                placeholder="请选择排序字段"
              >
                <el-option
            ...
            <template v-if="columnData.type == 2">
              ...
              <el-form-item label="左侧标题">
                <el-input
                  v-model="columnData.treeTitle"
                  placeholder="树形标题"
                ></el-input>
              </el-form-item>
              <el-form-item label="数据来源">
                <el-select
                  v-model="columnData.treeDataSource"
                  placeholder="请选择数据来源"
                >
              ...
              <template v-if="columnData.treeDataSource === 'dictionary'">
                <el-form-item label="数据字典">
                  <DicSelect
                    v-model="columnData.treeDictionary"
                    placeholder="请选择数据字典"
                  />
                </el-form-item>
                <el-form-item label="关联字段">
                  <el-select
                    v-model="columnData.treeDbTableFieldRelation"
                    placeholder="请选择关联字段"
                  >
                    <el-option
                      :label="item.__config__.label"
              ...
              <template v-if="columnData.treeDataSource === 'api'">
                <el-form-item label="数据选择">
                  <el-select
                    v-model="columnData.treeDbTable"
                    placeholder="请选择数据选择"
                  >
                    <el-option label="数据字典" value="dictionary"></el-option>
                    <el-option label="数据列表" value="api"></el-option>
                ...
                <el-form-item label="主键字段">
                  <el-select
                    v-model="columnData.treeDbTableField"
                    placeholder="请选择主键字段"
                  >
                    <el-option label="数据字典" value="dictionary"></el-option>
                    <el-option label="数据列表" value="api"></el-option>
                ...
                <el-form-item label="主键字段">
                  <el-select
                    v-model="columnData.treeDbTableFieldParentId"
                    placeholder="请选择主键字段"
                  >
                    <el-option label="数据字典" value="dictionary"></el-option>
                    <el-option label="数据列表" value="api"></el-option>
                ...
                <el-form-item label="显示字段">
                  <el-select
                    v-model="columnData.treeDbTableFieldShow"
                    placeholder="请选择显示字段"
                  >
                    <el-option label="数据字典" value="dictionary"></el-option>
                    <el-option label="数据列表" value="api"></el-option>
            ...
            >
              <el-checkbox
                :label="item.value"
                border
                v-for="item in columnBtnsOption"
                :key="item.value"
              >
                <i :class="item.icon"></i>
                <el-input v-model="item.label" size="mini"></el-input>
...
const defaultColumnData = {
  ...
  groupField: "", // 分组字段
  btnsList: [{ value: "add", icon: "el-icon-plus", label: "新增" }], // 按钮
  columnBtnsList: [
...
export default {
  ...
  created() {
    if (this.modelType == 4) {
      this.columnBtnsOption = this.columnBtnsOption.filter(
        (o) => o.value != "detail"
      );
    ...
    const loop = (data, parent) => {
      ...
      if (data.__config__ && data.__config__.ccitKey) {
    ...
    loop(getDrawingList());
    this.list = list;
    let columnOptions = list.filter(
      (o) => noColumnShowList.indexOf(o.__config__.ccitKey) < 0
    );
    let searchOptions = list.filter(
      (o) => noSearchList.indexOf(o.__config__.ccitKey) < 0
    );
    ...
    this.setValue(
      this.columnData.columnBtnsList,
  ...
  methods: {
    ...
    getData() {
      return new Promise((resolve, reject) => {
        ...
        if (this.columnData.type == 2) {
          if (!this.columnData.treeDictionary)
            reject({ msg: "请选择数据字典", target: 2 });
          if (!this.columnData.treeDbTableFieldRelation)
            reject({ msg: "请选择关联字段", target: 2 });
        }
    ...
    setSort() {
      ...
      this.sortable = Sortable.create(el, {
        ...
        onEnd: (evt) => {
          const targetRow = this.columnData.columnList.splice(
            evt.oldIndex,
            1
          )[0];
      ...
      const el2 = this.$refs.dragTableSearch.$el.querySelectorAll(
        ".el-table__body-wrapper > table > tbody"
      )[0];
      Sortable.create(el2, {
        ghostClass: "sortable-ghost", // Class name for the drop placeholder,
        ...
        onEnd: (evt) => {
          const targetRow = this.columnData.searchList.splice(
            evt.oldIndex,
            1
          )[0];
    ...
    handleClipboard(e) {
      ...
      this.columnData.columnList.forEach((item) => {
        columnCopy += `{"fullName": "${item.label}", "enCode": "${item.prop}", "enabledMark": "1"},`;
      });
      columnCopy = columnCopy.substr(0, columnCopy.length - 1);
      columnCopy += ']'
      clipboard(columnCopy, e);
    },
  },
};
</script>
