...
<template>
  ...
  >
    ...
    <header
      slot="title"
      class="header"
      v-if="value && value.type == 'condition' && properties.title"
    >
      <el-input
        size="mini"
    ...
    <section class="condition-pane pd-10" v-if="value && isTimerNode()">
      <el-row style="padding: 10px">
        <el-col
          :span="24"
          style="font-size: 14px; line-height: 32px; color: #a5a5a5"
        >
      ...
      <el-tabs style="height: 100%">
        <el-tab-pane label="发起人">
          <el-row style="padding: 10px">
            ...
            <el-col
              :span="20"
              style="font-size: 14px; line-height: 32px; margin-bottom: 10px"
            >
              默认所有人,需要设置请选择
            </el-col>
            <div class="option-box" style="border-bottom: 1px solid #dcdfe6; padding-bottom: 1rem">
              <org-select
            ...
            <br/>
            <el-col :span="7" style="font-size: 14px; line-height: 32px">发起人绑定表单字段</el-col>
            <el-col :span="17" style="font-size: 14px; line-height: 32px; margin-bottom: 10px">该字段值必须为员工ID或者员工工号</el-col>
            <el-select v-model="initiateFiled" placeholder="请选择" style="margin:10px 0 0 5px; width:95%" clearable>
              <el-option
          ...
          <el-form label-position="top" class="pd-10">
            ...
            <template v-if="properties.hasInitFunc">
              <el-form-item label="发起事件请求路径">
                <el-input
                  v-model="properties.initInterfaceUrl"
                  placeholder="请输入接口地址"
                />
              </el-form-item>
            </template>
            ...
            <template v-if="properties.hasEndFunc">
              <el-form-item label="结束事件请求路径">
                <el-input
                  v-model="properties.endInterfaceUrl"
                  placeholder="请输入接口地址"
                />
              </el-form-item>
            </template>
            ...
            <template v-if="properties.hasFlowRecallFunc">
              <el-form-item label="撤回事件请求路径">
                <el-input
                  v-model="properties.flowRecallInterfaceUrl"
                  placeholder="请输入接口地址"
                />
              </el-form-item>
            </template>
        ...
        <el-tab-pane label="审批人" name="config">
          <div>
            ...
            <div style="border-bottom: 1px solid #dcdfe6; padding-bottom: 1rem">
              <div
                v-if="approverForm.assigneeType === 3"
                class="option-box"
                style="color: #a5a5a5"
              >
                发起人自己将作为审批人处理审批单
              ...
              <div
                v-else-if="approverForm.assigneeType === 2"
                class="option-box"
                style="color: #a5a5a5"
              >
                发起人的部门主管将作为审批人处理审批单
              ...
              <div
                v-else-if="approverForm.assigneeType === 1"
                class="option-box"
                style="color: #a5a5a5"
              >
                发起人主管将作为审批人处理审批单
              ...
              <div
                v-else-if="approverForm.assigneeType === 7"
                class="option-box"
                style="color: #a5a5a5"
              >
                将由流程表单中的人员进行审批
              ...
              <div
                v-else-if="approverForm.assigneeType === 9"
                class="option-box"
                style="color: #a5a5a5"
              >
                发起人的公司领导将作为审批人处理审批单
              ...
              <div
                v-else-if="approverForm.assigneeType === 10"
                class="option-box"
              >
                <CCIT-TreeSelect
                  :options="dataInterfaceSelector"
                  v-model="approverForm.apiFlag"
                  placeholder="请选择数据接口"
                  lastLevel
                  lastLevelKey="categoryId"
                  lastLevelValue="1"
                  clearable
                  @getValue="getNodeInfo"
                />
              </div>
              <div v-else class="option-box">
                <org-select
                  ...
                  buttonType="button"
                  v-model="approverForm.approverPos"
                  title="添加岗位"
                  type="position"
                  @change="onOrgChange"
                  class="mb-10"
            ...
            <el-form label-position="top" :model="approverForm" class="pd-10">
              ...
              <el-form-item label="相同人相邻节点自动流转">
                <el-select
                  v-model="approverForm.samePeopelNodeAutoCirculation"
                  placeholder="请选择"
                  style="width: 100%"
                >
              ...
              <el-form-item label="相同人不同节点自动流转">
                <el-select
                  v-model="approverForm.samePeopelDiffrentNodeAutoCirculation"
                  placeholder="请选择"
                  style="width: 100%"
                >
                  <el-option label="否" :value="false"></el-option>
              ...
              <el-form-item label="驳回步骤">
                <el-select
                  v-model="approverForm.rejectStep"
                  placeholder="请选择"
                  style="width: 100%"
                >
                  <el-option
                    v-for="item in rejectStepOptions"
                    :key="item.nodeId"
              ...
              <el-form-item label="说明">
                <el-input
                  v-model="approverForm.description"
                  type="textarea"
                  :rows="3"
                ></el-input>
              </el-form-item>
          ...
          <div class="pd-10">
            <div class="option-box">
              <org-select
                ...
                buttonType="button"
                v-model="approverForm.circulatePosition"
                title="添加抄送岗位"
                type="position"
                @change="onOrgChange"
                class="mb-10"
              ...
              <org-select
                ...
                buttonType="button"
                v-model="approverForm.circulateUser"
                title="添加抄送用户"
                @change="onOrgChange"
              />
            </div>
          ...
          <el-form label-position="top" :model="approverForm" class="pd-10">
            ...
            <template v-if="approverForm.hasApproverFunc">
              <el-form-item label="审批事件请求路径">
                <el-input
                  v-model="approverForm.approverInterfaceUrl"
                  placeholder="请输入接口地址"
                />
              </el-form-item>
            </template>
            ...
            <template v-if="approverForm.hasRecallFunc">
              <el-form-item label="撤回事件请求路径">
                <el-input
                  v-model="approverForm.recallInterfaceUrl"
                  placeholder="请输入接口地址"
                />
              </el-form-item>
            </template>
...
const defaultApproverForm = {
  ...
  hasRecallFunc: false,
  recallInterfaceUrl: "",
  samePeopelNodeAutoCirculation: false, // 相同人相同节点自动流转
  samePeopelDiffrentNodeAutoCirculation: false, // 相同人不同节点自动流转
};
const defaultStep = [
  {
    nodeId: "1",
  ...
  computed: {
    ...
    usedFormItems() {
      ...
      const loop = (data, parent) => {
        ...
        if (data.__vModel__ && data.__config__.ccitKey !== "table")
          list.push(data);
      };
      loop(getDrawingList());
  ...
  methods: {
    ...
    conditionNodeComfirm() {
      ...
      for (let i = 0; i < this.pconditions.length; i++) {
        ...
        nodeContent +=
          `[${e.fieldName} ${e.symbol} ${e.filedValue}] ${
            i + 1 == this.pconditions.length ? "" : e.logicName
          }` + "\n";
      }
    ...
    startNodeComfirm() {
      ...
      this.properties.initiateFiled = this.initiateFiled;
      let content = "",
      ...
      if (!initiatorText && !initiatorPosText) {
        ...
        content =
          initiatorPosText +
          (initiatorText && initiatorPosText ? "," : "") +
          initiatorText;
      }
    ...
    timerNodeComfirm() {
      ...
      if (!day && !hour && !minute && !second) {
        ...
        content = `将于${day ? day + "天" : ""}${hour ? hour + "小时" : ""}${
          minute ? minute + "分钟" : ""
        }${second ? second + "秒" : ""}后流转`;
      }
      this.$emit("confirm", this.properties, content);
    ...
    approverNodeComfirm() {
      ...
      if ([6, 8].includes(assigneeType)) {
        if (
          ...
          !this.approverForm.approverPos.length
        ) {
          this.$message({
            message: "请设置审批人",
            type: "error",
    ...
    initInitiator() {
      ...
      this.initiatePos =
        this.value.properties && this.value.properties.initiatePos;
        this.initiateFiled = this.value.properties && this.value.properties.initiateFiled;
    ...
    getNodeInfo(val) {
      if (val) {
        getDataInterfaceInfo(val)
            .then((res) => {
              if (res.code == 200) {
                let fullName = res.data.fullName || "";
                this.apiContent = "在线接口(" + fullName + ")";
              }
            })
            .catch((res) => {
              console.log(res);
  ...
  watch: {
    ...
    value(newVal) {
      if (newVal && newVal.properties) {
        if (newVal.properties.apiFlag != "") {
          this.getNodeInfo(newVal.properties.apiFlag);
        }
        this.visible = true;
