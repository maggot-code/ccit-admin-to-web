...
function NodeFactory(ctx, data, h) {
  ...
  let res = [],
    branchNode = "",
    content = NodeUtils.isConditionNode(data) ? (NodeUtils.verifyConditionField(data, errorFiled) && errorFiled.length > 0 ? `条件字段【${errorFiled.join(",")}】发生变更,请删除后重新设置` : "未设置条件") : "未设置审批人",
    selfNode = (
      <div class="node-wrap">
