...
export class NodeUtils {
  ...
  /**
   * 检查表单数据字段和流程判断条件字段是否存在差异
   */
  static verifyConditionField = (node, errorFiled) => {
    let filedList = this.usedFormItems();
    //判断条件字段是否存在
    const existField = (condition) => {
      let flag = false;
      for (let o of filedList) {
        if (o.__config__.label == condition.fieldName && o.__vModel__ == condition.field) {
          flag = true;
          break;
        }
      }
    ...
    let conditions = node.properties.conditions;
    if (!isEmptyArray(conditions)) {
      let flag = false;
      for (let condition of conditions) {
        if (!existField(condition)) {//如果有一条条件字段不存在，则验证失败
          flag = true;
          if (errorFiled) {
            errorFiled.push(condition.fieldName);
          }
        }
      }
      return flag
  ...
  static checkNode(node, parent) {
    ...
    this.isConditionNode(node) &&
      ...
      this.verifyConditionField(node) &&
      (valid = false)

    const customSettings = [1, 2, 3, 7, 9]
    this.isApproverNode(node) &&
      !customSettings.includes(props.assigneeType) &&
      (isEmptyArray(props.approvers) && isEmptyArray(props.approverPos)) && (isEmpty(props.apiFlag)) &&
      (valid = false)
    return valid
  }
  /**
  ...
  static usedFormItems() {
    ...
    const loop = (data, parent) => {
      ...
      if (data.__vModel__ && data.__config__.ccitKey !== 'table') list.push(data)
    }
    loop(getDrawingList())
    return list
  }
