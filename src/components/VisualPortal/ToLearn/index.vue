        ...
        <div class="portal-box-body todolist" :style="bodyStyle" v-loading="loading">
            <div class="innerTabs" v-if="showOuter">
                <el-tabs v-model="activeName" @tab-click="handleClick">
                    <el-tab-pane :label="todoList" name="daixue">
                        ...
                        <div v-else>
                            ...
                            <div class="listtext">暂时没有待学习事项...</div>
                        </div>

                    </el-tab-pane>
                    <el-tab-pane :label="'已学'" name="yixue">
                        ...
                        <div v-else>
                            <div class="nolist"></div>
                            <div class="listtext">暂时没有已学习事项...</div>
                        </div>
                    </el-tab-pane>
                    <el-tab-pane :label="'收藏'" name="save3">
                        ...
                        <div v-else>
                            <div class="nolist"></div>
                            <div class="listtext">暂时没有收藏学习事项...</div>
                        </div>
...
import { getMyToDoList, toBeLearn, getUser } from '@/api/todolist/index.js'
export default {
    mixins: [mixin],
    props: {
    ...
    methods: {
                                ...
                                toBeLearn(
                                    {
                                        ...
                                        (res) => {
                                            if (res.code == 200) {
                                                ...
                                                document.getElementById("tab-daixue").innerHTML = `待学(<span style="color:red">${res.data.dyNums}</span>)`
                                            }
                                        }
                                    )
                                toBeLearn(
        ...
        jumpToMore() {
            let url = ''
            if (this.activeName == 'daixue') {
                url = `http://oam.chinacoal.com/unionTaskWS/learnListUAT.jsp?userCode=${this.userInfo.data.id}&state=0`
            } else if (this.activeName == 'yixue') {
                url = `http://oam.chinacoal.com/unionTaskWS/learnListUAT.jsp?userCode=${this.userInfo.data.id}&state=2`
            } else if (this.activeName == 'save3') {
                url = `http://oam.chinacoal.com/unionTaskWS/learnListUAT.jsp?userCode=${this.userInfo.data.id}&state=7`
            }
            window.open(url)
        }
    },
