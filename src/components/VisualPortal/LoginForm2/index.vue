<template>
  <div class="login-form" :style="bgStyle">
    <img class="login-logo" :src="downLoadUrl" alt="" />
    ...
    >
      <el-form-item
        ...
        prop="account"
        :class="accountSts ? 'inputfocus' : 'defalut-input'"
      >
        <el-input
          ref="account"
          v-model="loginForm.account"
          placeholder="8位数字员工编号"
          name="account"
          type="text"
          tabindex="1"
          autocomplete="on"
          size="large"
          :style="formItemStyle(0)"
          @blur="onBlur('accountSts')"
          @focus="onFocus('accountSts')"
        >
          <template slot="prepend"
            ><img v-if="accountSts" src="@/assets/images/visualPortal/loginForm/user.png" />
            <img v-else src="@/assets/images/visualPortal/loginForm/user-gray.png" />
            <div
              class="divide"
              :class="accountSts ? 'divide-active' : ''"
            ></div> </template
        ></el-input>
      </el-form-item>
      ...
      >
        <el-form-item
          style="margin-bottom: 5%"
          prop="password"
          :class="passwordSts ? 'inputfocus' : 'defalut-input'"
        >
          <el-input
            ref="password"
            ...
            size="large"
            :style="formItemStyle(1)"
            ><template slot="prepend">
              <img v-if="passwordSts" src="@/assets/images/visualPortal/loginForm/pwd.png" />
              <img v-else src="@/assets/images/visualPortal/loginForm/pwd-gray.png" />
              <div
                class="divide"
                :class="passwordSts ? 'divide-active' : ''"
              ></div></template
          ></el-input>
        </el-form-item>
      </el-tooltip>
      <el-form-item v-if="useVerifiCode" prop="code">
        <el-row>
          <el-col :span="18">
            <el-input
              ref="code"
      ...
      <div class="tips">
        ...
        <span>登录问题可联系技术支持，电话010-82256543。</span>
      </div>
      <!-- <div class="foot">
  ...
  computed: {
    downLoadUrl() {
      if (!this.logoObject.fileId)
        return require("@/assets/images/visualPortal/loginForm/logo.png");
      else {
        return (
          this.define.comUrl + "api/file/Image/annex/" + this.logoObject.fileId
        );
      }
    },
    formItemStyle() {
  ...
  methods: {
    ...
    checkPassword() {
      getSysInfoVal("defaultPwd").then((res) => {
        ...
        if (this.loginForm.password === defaultPwd) {
          this.$confirm(
