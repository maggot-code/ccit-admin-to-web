    ...
    <el-card :class="['portal-box', titleColor]" :style="cardStyle">
        <div :slot="headerSlot" class="portal-common-title" :style="titleStyle">
            ...
            <div class="right">
                ...
                <span v-if="item.showFresh" title="刷新" class="item">
                    <span style="color:#999;font-size:14px;font-weight:500;cursor: pointer;" class="titleIconhover" @click="getData('')">刷新</span>
                </span>
                <span v-if="item.showMore" title="更多" class="item">
                    <span style="color:#999;font-size:14px;font-weight:500;cursor: pointer;" class="titleIconhover" @click="jumpToMore">更多</span>
                </span>
            </div>
        </div>
        <div class="allcheck" @click="toggleSelection(toDoList.tasks)" v-show="!loading">全选</div>
        <div class="allread" ref='allread' @click="toggleSelectionChecked(toDoList.tasks)" v-show="!loading">已阅</div>
                            ...
                            <el-table ref="multipleTable" :data="toDoList.tasks" tooltip-effect="dark" style="width: 100%" :show-header="false" @selection-change="handleSelectionChange">
                                ...
                                <el-table-column label="标题">
                                    <template slot-scope="scope">
                                        <div class="tabletext" @click="formatUrl(toDoList,item,toDoList.state)">{{ scope.row.taskName }}</div>
                                    </template>
                                </el-table-column>
                                <el-table-column label="领导待阅" width="70px">
                                    <template slot-scope="scope">
                                        <div class="leaderFlag" @click="formatUrl(toDoList,item,toDoList.state)" v-if="scope.row.leaderFlag?'领导待阅':''">领导待阅</div>
                                    </template>
                                </el-table-column>
                                <el-table-column label="日期" width="90px">
                                    <template slot-scope="scope">
                                        <div class="tabletime" @click="formatUrl(toDoList,item,toDoList.state)">{{ scope.row.handleTime }}</div>
                                    </template>
                                </el-table-column>
                            </el-table>
                        </div>
