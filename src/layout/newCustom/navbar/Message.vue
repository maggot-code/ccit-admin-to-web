<template>
  ...
  >
    <el-scrollbar class="msg-box">
      ...
      >
        <div>
          <img v-if="item.type === 5" src="@/assets/images/msg-icon2.png" />
          <img v-else src="@/assets/images/msg-icon1.png" />
    ...
    <div class="clean">
      <el-button type="text" @click="cleanMsg">清空消息</el-button>
    </div>
    <el-badge slot="reference" :value="messageCount" :hidden="!messageCount">
      <i class="icon-ym icon-ym-header-message icon"></i>
    </el-badge>
  </el-popover>
</template>
<script>
      ...
      ReadInfo(data.id).then((res) => {
        ...
        if (data.type === 4) {
          ...
          this.$router.push({
            path: "/wdrc1",
            query: {
              // modelId: "ca5ac9abff02477fada2a1cf690db816",
            },
          });
        } else if (data.type === 5) {
          // 督办的待办中心
          this.$router.push({
            path: "/djbrw",
            query: {
              modelId: "bec59939ef144f9e836a949a01177b97",
            },
          });
        } else if (data.type === 6) {
          // 督办的任务中心
          this.$router.push({
            path: "/db_0003",
            query: {
              modelId: "6c2a5505bad44045bdf6e67f8977fc0b",
            },
          });
        }
      });
