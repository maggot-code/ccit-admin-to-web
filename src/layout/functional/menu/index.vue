...
export default {
  ...
  created() {
    ...
    .then((res) => {
      let webList = this.menuList.filter((menu) => {
        return res.data.list.some((item) => {
      ...
      erd.listenTo(document.getElementById('topMenu'), function (element) {
        ...
        let length = webList.length
        if (number >= length) return _this.list = webList
        for (let i = 0; i < length; i++) {
          const item = webList[i];
          if (i < number - 1) {
            newList.push(item)
          } else {
