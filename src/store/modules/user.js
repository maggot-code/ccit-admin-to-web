...
const actions = {
  ...
  login({ commit }, userInfo) {
    ...
    return new Promise((resolve, reject) => {
      login({ account: account.trim(), password: md5(password), code: code.trim(), timestamp: timestamp, type: type }).then(response => {
        const { data } = response
        const layoutList = ['classic', 'functional', 'plain', 'custom', 'custom2']
      ...
      getInfo().then(response => {
        ...
        function setData(list) {
          for (let i = 0; i < list.length; i++) {
            ...
            if (e.type == 7) {
              let propertyJson = e.propertyJson ? JSON.parse(e.propertyJson) : null;
              let paramsDataJson = e.paramsData ? JSON.parse(e.paramsData) : null;
              let params = [];
              let token = propertyJson.showTokenType ? getToken() : getToken().split(" ")[1];
