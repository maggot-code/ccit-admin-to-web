...
import { getDimension } from '@/api/system/dimension'
...
const mutations = {
  ...
  SET_DIMENSION_LIST: (state, dimensionList) => {
    state.dimensionList = dimensionList
  },
  SET_CURRENT_DIMENSION: (state, currentDimension) => {
...
const actions = {
  ...
  getDimensionList({state, commit}) {
    return new Promise(async resolve => {
      if (!state.dimensionList.length) {
        getDimension().then(res => {
          commit('SET_DIMENSION_LIST', res.data.list)
          if(!state.currentDimension && res.data.list) {
            commit('SET_CURRENT_DIMENSION', res.data.list[0].encode)
          }
          resolve(res.data.list)
        }).catch(error => {
          reject(error)
