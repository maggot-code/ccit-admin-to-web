<template>
  <div class="CCIT-common-layout">
            ...
            <template slot-scope="scope" v-if="!scope.row.top">
              <div>
                <el-switch
                  ...
                  :active-value="1"
                  :inactive-value="0"
                  @click.native="enablePortal(scope.row)"
                />
              </div>
            </template>
          </el-table-column>
    ...
    <Form
      ...
      @close="closeForm"
    />
    <Preview :visible.sync="loginPreviewVisible" :portalId="portalId" />
...
import Preview from "../loginPortal/Preview";
...
export default {
  ...
  methods: {
    ...
    getDictionaryData() {
      this.$store.dispatch("base/getDictionaryData", { sort: 'loginPortalDesigner' }).then((res) => {
    ...
    enablePortal(row) {
      ...
      this.$confirm(`您确定要${txt}当前门户作为默认登录页吗?`, "提示", {
        type: "warning",
      })
        .then(() => {
