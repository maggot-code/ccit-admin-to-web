    ...
    <div class="CCIT-preview-main">
      ...
      >
        ...
        <el-form-item label="正文" prop="bodyText">
          <CCIT-Quill v-model="dataForm.bodyText" />
        </el-form-item>
      </el-form>
    </div>
  </transition>
...
export default {
  ...
  methods: {
      ...
      this.$refs["dataForm"].validate((valid) => {
        if (valid) {
          ...
          let list = this.fileList.map((o) => ({
            ...
            fileSize: o.fileSize || this.ccit.toFileSize(o.raw.size),
          ...
          if (this.showBCC)
            data = { ...data, bcc: this.dataForm.bcc.join(",") };
    ...
    recipientChange(e) {
      if (e.length) {
        let str = e[e.length - 1];
        if (!validEmail(str)) {
          this.$message({ message: "请输入正确的邮箱格式", type: "warning" });
          this.dataForm.recipient.splice(this.dataForm.recipient.length - 1, 1);
        }
      }
    },
  },
