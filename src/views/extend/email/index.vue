  ...
  <div class="app-container Email-container nohead">
    ...
    >
      ...
      <el-tab-pane name="inBox">
        <span slot="label"
          ><i class="icon-ym icon-ym-extend-envelope"></i>收件箱</span
        >
      </el-tab-pane>
      <el-tab-pane name="star">
        <span slot="label"
          ><i class="icon-ym icon-ym-extend-star"></i>星标件</span
        >
      </el-tab-pane>
      <el-tab-pane name="draft">
        <span slot="label"
          ><i class="icon-ym icon-ym-extend-exclamation-triangle"></i
          >草稿箱</span
        >
      </el-tab-pane>
      <el-tab-pane name="sent">
        <span slot="label"
          ><i class="icon-ym icon-ym-extend-paper-plane"></i>已发送</span
        >
      </el-tab-pane>
      <el-tab-pane name="setAccount">
        <span slot="label"
          ><i class="icon-ym icon-ym-extend-cog"></i>邮箱配置</span
        >
      </el-tab-pane>
      ...
      <div class="main CCIT-flex-main">
        <el-row class="CCIT-common-search-box" :gutter="16">
          ...
          <div class="CCIT-common-search-box-right">
            <el-button
              type="primary"
              icon="el-icon-download"
              @click="receiveEmail('increment')"
              :loading="receiveingIncrement"
              >最近邮件</el-button
            >
            <el-button
              type="primary"
              icon="el-icon-download"
              @click="receiveEmail('total')"
              :loading="receiveingTotal"
              >全部邮件</el-button
            >
          </div>
        </el-row>
        <CCIT-table v-loading="listLoading" :data="list">
          <el-table-column label="已读" width="50" align="center">
            <template slot-scope="scope">
              <template v-if="activeTab === 'inBox' || activeTab === 'star'">
                <span v-if="scope.row.isRead">
                  <i
                    class="icon-ym icon-ym-extend-envelope-open-o i-default"
                    title="点击标记为未读"
                    @click="ReceiveUnread(scope.row)"
                  ></i
                ></span>
                <span v-else
                  ><i
                    class="icon-ym icon-ym-extend-envelope text-warning"
                    title="点击标记为已读"
                    @click="ReceiveRead(scope.row)"
                  ></i
                ></span>
              </template>
              <span v-else
                ><i class="icon-ym icon-ym-extend-envelope-open-o i-default"></i
              ></span>
            </template>
          </el-table-column>
          <el-table-column label="附件" width="50" prop="attachment" align="center">
            <template slot-scope="scope">
              <span
                v-if="
                  scope.row.attachment &&
                  JSON.parse(scope.row.attachment).length
                "
                ><i class="icon-ym icon-ym-extend-paperclip i-default"></i
              ></span>
            </template>
          </el-table-column>
          <el-table-column
          ...
          <el-table-column prop="subject" label="主题" show-overflow-tooltip>
            <template slot-scope="scope">
              ...
              <el-link
                @click="
                  readInfo(scope.row.id, activeTab === 'sent', scope.$index)
                "
                style="font-size: 12px"
                v-else
              >
          ...
          >
            <template slot-scope="scope">
              <span v-if="scope.row.starred"
                ><i
                  class="icon-ym icon-ym-extend-star text-warning img-star"
                  title="点击取消星标"
                  @click="ReceiveNoStarred(scope.row, scope.$index)"
                ></i
              ></span>
              <span v-else
                ><i
                  class="icon-ym icon-ym-extend-star-o i-default img-star"
                  title="点击标记为星标邮件"
                  @click="ReceiveYesStarred(scope.row)"
                ></i
              ></span>
            </template>
          </el-table-column>
    ...
    >
      <el-progress
        :stroke-width="10"
        :percentage="receiveInfo.percentage"
        :status="receiveInfo.percentage === 100 ? 'success' : undefined"
      ></el-progress>
      <div class="info">
        <span>下载数：{{ receiveInfo.curCount }}</span>
        <span class="count">总邮件数：{{ receiveInfo.count }}</span>
        <template
          ><span class="right" v-if="receiveInfo.percentage == 0"
            >准备就绪...</span
          >
          <span class="right" v-else-if="receiveInfo.percentage == 100"
            >下载完成！</span
          >
          <span v-else class="right title"
            >当前下载邮件：《{{ receiveInfo.title }}》</span
          ></template
        >
      </div>
    </el-dialog>
...
export default {
  ...
  data() {
    return {
      receiveingIncrement: false,
      receiveingTotal: false,
      configVisible: false,
      formVisible: false,
      detailVisible: false,
      subordinateIds: [],
  ...
  watch: {
    ...
    'receiveInfo.percentage'(val) {
      if(val === 100) {
        this.$message.success(`收件成功${this.receiveInfo.curCount}条`);
      }
    },
    // 处理门户或消息跳转过来的操作
    $route: {
      handler(to, from) {
        if (
          to.name === this.$options.name &&
          (to.params.comp === "mailBox" || to.params.comp === "message")
        ) {
          let params = to.params;
          if (params.type === "draft") {
            this.write(params.id);
          } else if (params.type === "write") {
            this.write();
          } else if (params.type === "inBox" || params.type === "sent") {
            let isSend = params.type === 'sent'
            this.readInfo(params.id, isSend, params.index);
          } else {
            return;
          }
        } else {
  ...
  methods: {
    ...
    receiveEmail(type) {
      if (type === "increment") {
        this.receiveingIncrement = true;
      } else {
        this.receiveingTotal = true;
      }
      var interval = setInterval(() => {
        this.initWebSocket();
        if (this.receiveInfo.percentage === 100) {
          clearInterval(interval);
          if (type === "increment") {
            this.receiveingIncrement = false;
          } else {
            this.receiveingTotal = false;
          }
        }
      }, 100);
      this.dialogVisible = true;
      if (this.receiveInfo.percentage == 0) {
        Receive(type)
          .then((res) => {
            this.receiveingIncrement = false;
            this.receiveingTotal = false;
    ...
    initWebSocket() {
      ...
      if ("WebSocket" in window) {
        ...
        socket.onmessage = (event) => {
          ...
          if (data.method == "messageProcess") {
            let percentage = 0;
            if (data.count == 0) {
              percentage = 100;
            } else {
              percentage = parseInt(
                ((data.curCount / data.count) * 100).toFixed(0)
              );
            }
            this.receiveInfo = {
              count: data.count,
