          ...
          <template v-if="showAll">
            ...
            <el-col :span="6">
              <el-form-item label="所属流程">
                ...
                >
                  <el-option-group
                    v-for="group in flowEngineListAll"
                    :key="group.id"
    ...
    $route: {
      handler(to, from) {
        if (
          ...
          to.params.comp === "todoList" &&
          to.params.type === "detail"
        ) {
          this.toDetail(to.params);
        } else {
          if (this.formVisible) {
    ...
    getFlowEngineList() {
      FlowEngineListAll().then((res) => {
        ...
        for (let i = 0; i < this.flowEngineListAll.length; i++) {
          let child = this.flowEngineList.filter(
