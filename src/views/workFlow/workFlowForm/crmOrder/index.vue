        ...
        <el-row>
          ...
          <el-col :span="6" v-if="JudgeShow('salesmanId')">
            <el-form-item label="业务人员" prop="salesmanId">
              <User-TreeSelect v-model="dataForm.salesmanId"
                @getValue="getValue" placeholder="选择人员" :disabled="JudgeWrite('salesmanId')">
              </User-TreeSelect>
            </el-form-item>
          </el-col>
...
export default {
  ...
  methods: {
    ...
    count(row) {
      //金额 = 数量*单价
      row.amount = this.ccit.toDecimal(parseFloat(row.price) * parseFloat(row.qty))
      //折扣价 = (单价*折扣)
      var discountPrice = row.price * (row.discount / 100);
      //实际单价 = 折扣价 * (1 + (税率 / 100))
      row.actualPrice = this.ccit.toDecimal(discountPrice * (1 + (row.cess / 100)));
      //实际金额
      row.actualAmount = this.ccit.toDecimal(parseFloat(row.actualPrice) * parseFloat(row.qty))
    }
  }
}
</script>
