<template>
  <div class="CCIT-common-layout">
          ...
          <template v-if="showAll">
            ...
            <el-col :span="6">
              <el-form-item label="发起人员">
                <el-select v-model="creatorUserId" placeholder="选择发起人员" clearable filterable>
      ...
      <div class="CCIT-common-layout-main CCIT-flex-main">
        ...
        <CCIT-table v-loading="listLoading" :data="list">
          <el-table-column prop="fullName" label="流程标题" show-overflow-tooltip
            v-if="ccit.hasP('fullName')" min-width="150" />
          <el-table-column prop="flowName" label="所属流程" width="130" v-if="ccit.hasP('flowName')" />
          <el-table-column prop="startTime" label="发起时间" width="130"
            :formatter="ccit.tableDateFormat" v-if="ccit.hasP('startTime')" />
          <el-table-column prop="userName" label="发起人员" width="130" v-if="ccit.hasP('userName')" />
          <el-table-column prop="flowUrgent" label="紧急程度" sortable width="130"
            v-if="ccit.hasP('flowUrgent')">
            <template slot-scope="scope">
              {{ scope.row.flowUrgent | urgentText() }}
            </template>
          </el-table-column>
          <el-table-column prop="status" label="流程状态" width="130" v-if="ccit.hasP('status')">
            <template slot-scope="scope">
    ...
    <fromBox v-if="formVisible" ref="CCITForm" @close="colseForm" />
  </div>
</template>
