...
export default {
  data() {
    return {
      tabIndex: 0,
      tabs: [
        { id: 0, name: "首页", path: "/introduce" },
        { id: 1, name: "产品介绍", path: "/introduce/product" },
        { id: 2, name: "关于我们", path: "/introduce/aboutUs" },
      ],
    };
  },
  mounted() {
    let that = this;
    setTimeout(function () {
      if (that.$route.path === "/introduce/main") {
        that.tabIndex = 0;
      } else if (
        that.$route.path === "/introduce/product" ||
        that.$route.path === "/introduce/system"
      ) {
        that.tabIndex = 1;
      } else {
        that.tabIndex = 2;
  ...
  methods: {
    ...
    goHome() {
      this.$router.push("/introduce/main").catch((err) => {});
      this.tabIndex = 0;
    },
  },
};
