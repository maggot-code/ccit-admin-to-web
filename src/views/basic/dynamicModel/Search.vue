<template>
  <el-row class=" CCIT-common-search-box" :gutter="16" v-if="searchList.length">
    <el-form @submit.native.prevent>
      <el-col :span="6" v-for="(item, index) in searchList" :key="index">
        <el-form-item :label="item.__config__.label" v-show="index<3 || (index>=3 && showAll)">
          <template v-if="useInputList.indexOf(item.__config__.ccitKey)>-1">
            <el-input v-model="item.value" :placeholder="item.__config__.label" clearable
              class="item">
            </el-input>
          </template>
          <template v-else-if="useTimeList.indexOf(item.__config__.ccitKey)>-1">
            <el-date-picker v-model="item.value" placeholder="选择时间" value-format="timestamp"
              format="yyyy-MM-dd" class="item" />
          </template>
          <template v-else-if="useSelectList.indexOf(item.__config__.ccitKey)>-1">
          ...
          <template v-else>
            <template v-if="item.__config__.ccitKey==='cascader'">
            ...
            <template v-if="item.__config__.ccitKey==='time'">
            ...
            <template v-if="item.__config__.ccitKey==='timeRange'">
            ...
            <template v-if="item.__config__.ccitKey==='date'">
            ...
            <template v-if="item.__config__.ccitKey==='dateRange'">
            ...
            <template v-if="commonList.includes(item.__config__.ccitKey)">
              <component :is="item.__config__.tag" v-model="item.value"
                :placeholder="'请选择'+item.__config__.label" class="item" />
            </template>
            <template v-if="item.__config__.ccitKey==='address'">
              <CCITAddress v-model="item.value" :placeholder="'请选择'+item.__config__.label"
                :level="item.level" class="item" clearable />
            </template>
            <template v-if="item.__config__.ccitKey==='treeSelect'">
              <CCIT-TreeSelect v-model="item.value" :placeholder="'请选择'+item.__config__.label"
