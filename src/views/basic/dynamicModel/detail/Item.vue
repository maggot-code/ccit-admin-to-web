      ...
      <template v-else>
        <el-form-item :prop="item.__vModel__"
          ...
          <template v-else-if="item.__config__.ccitKey==='uploadImg'">
            <CCITenlarge :img="define.comUrl+cItem.url"
              v-for="(cItem,ci) in item.__config__.defaultValue" :key="ci" />
          </template>
          ...
          <template v-else-if="item.__config__.ccitKey==='switch'">
          ...
          <template v-else-if="item.__config__.ccitKey==='relationFormAttr'">
...
<script>
import CCITenlarge from "@/components/CCIT-enlarge"
import { getDownloadUrl } from '@/api/common'

export default {
  name: 'Item',
  components: { CCITenlarge },
  props: {
    item: {
      type: Object,
      required: true
