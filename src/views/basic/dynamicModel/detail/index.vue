  ...
  <div>
    <template v-if="formData.popupType==='general'">
      <el-dialog title="详情" :close-on-click-modal="false" :visible.sync="visible"
        class="CCIT-dialog CCIT-dialog_center" lock-scroll :width="formData.generalWidth"
        append-to-body>
      ...
      this.$nextTick(() => {
        if (this.dataForm.id) {
          getDataChange(modelId, this.dataForm.id).then(res => {
            ...
            let formValue = JSON.parse(this.dataForm.data)
            for (let val in formValue) {
              formValue[val] = formValue[val] && typeof(formValue[val]) == 'string' ? formValue[val].replaceAll("%22", "\"") : formValue[val]
            }
      ...
      for (let i = 0; i < realList.length; i++) {
        ...
        const loop = list => {
          for (let i = 0; i < list.length; i++) {
            ...
            if (list[i].__config__ && list[i].__config__.ccitKey !== 'table' && list[i].__config__.children && Array.isArray(list[i].__config__.children)) {
    ...
    fillFormData(form, data) {
      ...
      const loop = list => {
        for (let i = 0; i < list.length; i++) {
          let item = list[i]
          if (item.__vModel__) {
            if (item.__config__.ccitKey === 'relationForm') {
          ...
          if (item.__config__.ccitKey === 'relationFormAttr') relationFormAttrList.push(item)
          if (item.__config__ && item.__config__.ccitKey !== 'table' && item.__config__.children && Array.isArray(item.__config__.children)) {
