...
<template>
  <div class="CCIT-common-layout" v-show="showPage">
    <div
      class="CCIT-common-layout-left"
      v-if="columnData.type === 2 && columnData.treeDictionary"
    >
          ...
          >
            <template slot-scope="scope" v-if="!scope.row.top">
              <el-button
                ...
                :key="i"
                :class="{ 'CCIT-table-delBtn': item.value == 'remove' }"
                @click="
                  columnBtnsHandel(item.value, scope.row.id, scope.$index)
                "
                v-has="'btn_' + item.value"
                >{{ item.label }}</el-button
              >
