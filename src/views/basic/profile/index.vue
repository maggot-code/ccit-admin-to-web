...
<!--      <el-tab-pane v-if="layoutType !== 'custom'" label="人脸识别" name="face">-->
<!--        <Face ref="face" v-if="visible.face" />-->
<!--      </el-tab-pane>-->
      ...
      <el-tab-pane label="我的下属" name="subordinate">
        ...
        <el-row :gutter="12">
          <el-col
            :span="6"
            v-for="(item, i) in subordinateIds"
            :key="i"
      ...
      <!-- <el-tab-pane
        v-if="layoutType !== 'custom'"
        label="系统权限"
        name="authorize"
        class="el-tab-pane-authorize"
      >
...
export default {
  name: "profile",
  components: { UserInfo, Password, Authorize, SysLog, Face },
  data() {
    return {
      user: {},
      subordinateIds: [],
    ...
    $route: {
      handler(to) {
        if (to.name === this.$options.name) {
          if (to.params.page === "login") {
            this.activeTab = to.params.activeTab;
          }
        }
      },
      immediate: true,
